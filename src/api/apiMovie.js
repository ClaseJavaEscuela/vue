...
export const getMovies =async (pagina)=>{
  try {
    const res = await axiosMovies.get(`/movie/popular?language=es&api_key=${API_KEY}&page=${pagina}`);
...
export const getInfoMovie =async(idMovie)=>{
  try {
    const res = await axiosMovies.get(`/movie/${idMovie}?language=es&api_key=${API_KEY}`); 
